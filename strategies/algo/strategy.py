...
def get_nearest_expiry(client):
    ...
    nifty_opts = df[(df["name"] == "NIFTY") & (df["instrumenttype"].isin(["CE", "PE"]))]
    if nifty_opts.empty:
...
def place_market_order(client, symbol, transaction_type, quantity):
    ...
    if not LIVE_TRADING:
        logger.info(f"[DRY RUN] Would place market order: {transaction_type} {quantity} shares of {symbol}")
...
def run_strategy():
    ...
    R1, S1 = get_nifty_pivots(client)
    logger.info(f"Today's Pivot Levels -> R1: {R1:.2f} | S1: {S1:.2f}")
    
    # 2. Get Expiry
    target_expiry = get_nearest_expiry(client)
                ...
                if completed_ts != last_processed_timestamp:
                    ...
                    if not active_position and trades_today < 3 and current_time < ENTRY_CUTOFF_TIME:
                        if bullish_signal:
                            ...
                            opt_symbol = find_option_contract(client, target_expiry, atm_strike, "PE")
                            if opt_symbol:
                                logger.info(f"Bullish signal triggered on completed candle {completed_ts}. Selling ATM Put: {opt_symbol} at strike {atm_strike}")
                                resp = place_market_order(client, opt_symbol, "SELL", QUANTITY)
                                if resp["status"] == "success":
                                    active_position = {
                                        "type": "SHORT_PUT",
                            ...
                            atm_strike = int(round(nifty_close / 50) * 50)
                            opt_symbol = find_option_contract(client, target_expiry, atm_strike, "CE")
                            if opt_symbol:
